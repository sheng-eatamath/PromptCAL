...
from sklearn.metrics import calinski_harabasz_score as chs_score
from sklearn.metrics import silhouette_score
from sklearn.metrics import homogeneity_completeness_v_measure as v_measure
...
def cluster_acc(y_true, y_pred, return_confm=False, eps=1e-10):
    """
...
# -------------------------------
# My Mixed Eval Function
# -------------------------------
def my_mean_precision(confm, eps=1e-10):
    score = (confm/(eps+confm.sum(axis=1, keepdims=True))).diagonal()
    return np.mean(score)


def my_mean_recall(confm, eps=1e-10):
    score = (confm/(eps+confm.sum(axis=0, keepdims=True))).diagonal()
    return np.mean(score)


def my_class_split_statistics(confm):
    
    return

    
def my_mixed_eval(targets, preds, mask, features):
    """
    Evaluate clustering metrics on two subsets of data, as defined by the mask 'mask'
    (Mask usually corresponding to `Old' and `New' classes in GCD setting)
    :param targets: All ground truth labels
    :param preds: All predictions
    :param mask: Mask defining two subsets, is_labeled
    :return:
    """
    ...
    if mask.sum() == 0:  # All examples come from unlabelled classes
        unlabelled_sil = silhouette_score(features, preds)
        map_unlabeled = my_mean_precision(confm_unlabelled)
        mrc_unlabeled = my_mean_recall(confm_unlabelled)
        res = {
            'unlabelled_sil': unlabelled_sil,
        }
        return res, mask.mean()
    else:
        labelled_sil = silhouette_score(features[mask], targets[mask])
        unlabelled_sil = silhouette_score(features[~mask], preds[~mask])
        total_sil = silhouette_score(features, preds)
        res = {
            'labelled_sil': labelled_sil,
            'unlabelled_sil': unlabelled_sil,
            'total_sil': total_sil,
        }
        return res, mask.mean()
