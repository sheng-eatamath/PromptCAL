...
def subsample_classes(dataset, include_classes=(0, 1, 8, 9)):
    """ create a subset with sample-order retained which only contains @include_classes
    """
...
def get_cifar_100_datasets(train_transform, test_transform, train_classes=range(80),
    ...
    # Split into training and validation sets, NOTE: val set=alpha*labeled_training
    train_idxs, val_idxs = get_train_val_indices(train_dataset_labelled)
    train_dataset_labelled_split = subsample_dataset(deepcopy(train_dataset_labelled), train_idxs)
    val_dataset_labelled_split = subsample_dataset(deepcopy(train_dataset_labelled), val_idxs)
    val_dataset_labelled_split.transform = test_transform
...
def get_cifar_100_datasets_with_gcdval(train_transform, test_transform, train_classes=range(80), 
    ...
    # Split whole dataset by train classes
    train_dataset_labelled = subsample_classes(deepcopy(whole_training_set), include_classes=train_classes)
    # Subsample train classes into trainl
    subsample_indices = subsample_instances(train_dataset_labelled, prop_indices_to_subsample=prop_train_labels)
    train_dataset_labelled = subsample_dataset(train_dataset_labelled, subsample_indices)
